...
const mockAPI = {
  stats: {
    getDashboard: async () => ({
      stats: {
        total_orders: 156,
        total_revenue_usdt: 8945.50,
        total_profit_usdt: 3578.20,
        profit_margin: 40,
        total_users: 89,
        total_products: 24,
        total_categories: 6,
        ...
        vip_users: 12,
        today_orders: 8,
        today_revenue: 456.00,
        pending_orders: 3,
        avg_order_value: 57.34,
        top_products: [
          { name: "Test E 250", sold_count: 45, price_usdt: 65, purchase_price_usdt: 35 },
          { name: "Deca 300", sold_count: 38, price_usdt: 75, purchase_price_usdt: 40 },
          { name: "Anavar 50", sold_count: 32, price_usdt: 120, purchase_price_usdt: 70 },
          { name: "Tren A 100", sold_count: 28, price_usdt: 95, purchase_price_usdt: 55 },
          { name: "HGH 10IU", sold_count: 22, price_usdt: 250, purchase_price_usdt: 150 }
        ]
      }
    }),
    getAnalytics: async (days) => ({
      daily_sales: Array.from({ length: days }, (_, i) => {
        const date = new Date();
      ...
      category_sales: [
        { name: "Bulking", emoji: "💪", revenue: 3250, quantity: 52 },
        { name: "Cutting", emoji: "✂️", revenue: 2850, quantity: 45 },
        { name: "PCT", emoji: "🛡️", revenue: 1200, quantity: 38 },
        { name: "HGH", emoji: "🚀", revenue: 950, quantity: 12 },
        { name: "Peptides", emoji: "💉", revenue: 695.50, quantity: 9 }
      ],
  ...
  orders: {
    getAll: async () => ({
      orders: [
        { _id: '1', order_number: 'APZ-A1B2', created_at: new Date().toISOString(), total_usdt: 125, profit_usdt: 50, status: 'completed' },
        { _id: '2', order_number: 'APZ-C3D4', created_at: new Date().toISOString(), total_usdt: 85, profit_usdt: 34, status: 'paid' },
        { _id: '3', order_number: 'APZ-E5F6', created_at: new Date().toISOString(), total_usdt: 210, profit_usdt: 84, status: 'pending' },
        { _id: '4', order_number: 'APZ-G7H8', created_at: new Date().toISOString(), total_usdt: 95, profit_usdt: 38, status: 'completed' },
        { _id: '5', order_number: 'APZ-I9J0', created_at: new Date().toISOString(), total_usdt: 155, profit_usdt: 62, status: 'paid' }
      ]
    })
  },
  admin: {
    clearOrders: async () => ({ message: 'Orders cleared' }),
    clearUsers: async () => ({ message: 'Users cleared' })
  }
};

// Use mock API if real API not available
const statsAPI = window.statsAPI || mockAPI.stats;
const ordersAPI = window.ordersAPI || mockAPI.orders;
const adminAPI = window.adminAPI || mockAPI.admin;
